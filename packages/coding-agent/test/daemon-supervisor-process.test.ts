...
`,
		...
		try {
			...
			await waitForCondition(
				async () => {
					...
					return resumed.success === true;
				},
				`Session did not resume after abrupt worker kill\n${readDaemonLogs(agentDir)}`,
				20_000,
			);
			expect(resumed?.success).toBe(true);
			const resumedSummary = requireSummary(resumed?.success ? resumed.data : undefined);
	...
	it("becomes ready while a stale worker recovers in the background", async () => {
		...
		// relaunches, exposing the session under a fresh worker pid. Recovery
		// retries with backoff (~15s), so the wait must fit the global test
		// timeout alongside setup and teardown.
		...
		await waitForCondition(
			async () => {
				...
				const match = sessions.find(
					(summary) => summary.workerPid !== undefined && summary.workerPid !== blocker.pid,
				);
				adoptedPid = match?.workerPid;
				return adoptedPid !== undefined;
			},
			`Stale worker did not recover in the background\n${readDaemonLogs(agentDir)}`,
			20_000,
		);
		workerPids.add(adoptedPid as number);
		await client.request({ type: "shutdown" });
		client.close();
		await waitForSocketGone(socketPath);
	});
